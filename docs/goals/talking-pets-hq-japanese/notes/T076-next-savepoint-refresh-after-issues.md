# T076 next savepoint refresh after issue drafts

## 結論

T075のgood first issue下書きが増えたので、PR #9 merge後の次保存点PRは **HQ receipts T069-T076 only** に更新する。

T074は古いmanifestとして残す。このT076を次savepointの正本にする。
...
rg -n "T069|T070|T071|T072|T073|T074|T075|T076|recommend_outreach_first|pr9_merge_ready|outreach_today_copy_pack_ready|multilingual_claim_still_accurate|api_tts_opt_in_boundary_ready|good_first_issue_drafts_ready" docs/goals/talking-pets-hq-japanese/state.yaml docs/goals/talking-pets-hq-japanese/notes/T0{69,70,71,72,73,74,75,76}-*.md
git status --short
```
...
2. 送信後、T007へ送信ログを貼る。
3. 反応が出たらT075のgood first issue候補から1件だけ作るか判断する。
4. local TTSは、必要になったらT069のAで `npm install` だけ試す。

## 状態

done。

PR #9 merge、Issue作成、outreach送信、npm install、model download、API callはしていない。
