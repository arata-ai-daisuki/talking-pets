# T271 Multilingual Verification Boundary Watch

## Objective

T270のlocal TTS boundary watch後に、多言語検証の境界へ戻る。
...
- status: active
- result: pending
- summary: 次は多言語検証境界を、support wording変更、private evidence保存、model download、API callなしで再確認する。
- next: fallback-only / provider-specific claim gateを維持できたらoutreach waiting laneへhandoffする。
