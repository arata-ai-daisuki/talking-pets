...
Day 1 outreachはT086で送信済みになった。

また、PR #13でIrodori-TTS Server providerが追加されたため、このT061の `sherpa-onnx-nodeをnpm installするか、local TTSを保留してoutreach優先にするか` という判断は古くなった。

以後のlocal TTS判断はT087/T088へ引き継ぐ。
