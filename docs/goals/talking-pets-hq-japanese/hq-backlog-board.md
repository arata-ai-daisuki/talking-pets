...
| --- | --- | --- | --- |
| Local TTS next choice refresh | 歌澄 音羽 / 白瀬 怜奈 | Active | MeloTTS detect-only、sherpa optional install確認、provider feedback intakeを踏まえ、次の小PR候補とMaster承認境界を再評価する。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| Outreach send queue refresh | 星宮 未来 | README/SNS導線が現行化されたので、手動送信候補の優先順と「送ったら記録する場所」を再点検する。 | Ready-To-Send Queueが現行リンクを使い、返信待ちをblockerにしない形で次の手動候補が見える。 |
| Multilingual evidence watch | 言守 詞葉 | Minimal Multilingual Report FormとIssue導線がREADMEへ出たので、外部証跡が来た時の処理順を保つ。 | fallback-only / provider-specific / provider feedback intakeの記録順が一目で分かる。 |
