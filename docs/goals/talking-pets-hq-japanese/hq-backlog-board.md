...
| --- | --- | --- | --- |
| MeloTTS health-only helper skeleton | 歌澄 音羽 | Active | `scripts/tts-melotts.mjs --health` を追加し、install/downloadなしで外部runtimeのhealthだけ確認する。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| MeloTTS next integration choice | 歌澄 音羽 | health-only helperが入った後、monitor health integration、installer detect-only、provider feedbackのどれを先にするか選ぶ。 | 音声routingやinstall/downloadなしで、次PRの範囲が決まる。 |
