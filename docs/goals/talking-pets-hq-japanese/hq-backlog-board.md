...
| --- | --- | --- | --- |
| MeloTTS detect/connect-only implementation card | 歌澄 音羽 | Active | 実装するならhealth-only helper skeletonまで、install/downloadなしで小PR境界を固定する。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| MeloTTS health-only helper skeleton | 歌澄 音羽 | T177の境界が固まった後、`--health`だけのhelperならdownloadなしで検証できる。 | MeloTTS absentでもclean failureし、normal checksが通る。 |
