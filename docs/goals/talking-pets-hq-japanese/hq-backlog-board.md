...
- GoalBuddy看板: `http://goalbuddy.localhost:41737/talking-pets-hq-japanese-2/`
...
| --- | --- | --- | --- |
| sherpa optional npm install trial | 歌澄 音羽 | Active | T061由来の判断として、model downloadなしで`sherpa-onnx-node`のoptional installだけを確認する。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| MeloTTS external runtime helper design | 歌澄 音羽 | runtime/cache境界は固まったので、次はdownloadなしでCLI/server接続設計だけを切り出せる。 | installerは検出/案内まで、download/installは別承認としてカード化される。 |
