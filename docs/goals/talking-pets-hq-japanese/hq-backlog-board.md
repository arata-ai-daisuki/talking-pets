...
| --- | --- | --- | --- |
| Multilingual evidence refresh | 言守 詞葉 | Active | ko/zh dedicated provider evidenceの小テスト導線と報告フォームをprovider feedback captureに合わせる。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| README/SNS route tune-up | 星宮 未来 / 文月 栞里 | TTS設計と多言語検証が進んだので、外向き導線が過大claimなしで古くないか再点検する。 | Star導線、verification issue導線、SNS copyが現在地に合っている。 |
