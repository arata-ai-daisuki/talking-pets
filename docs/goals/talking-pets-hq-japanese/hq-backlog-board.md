...
| --- | --- | --- | --- |
| Multilingual intake follow-up | 言守 詞葉 | Active | 外部証跡が届いたら、handling orderに沿ってverification-statusとprovider intakeを更新する。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| Local TTS approval check | 歌澄 音羽 / 白瀬 怜奈 | scorecard gapはdesign-note質問へ落ちたので、実験へ進む場合はMaster承認範囲を確認する。 | dependency/model/APIなしで、承認カードだけを提示できる。 |
| Outreach reply intake | 星宮 未来 | OpenClaw/V1GPTはsentなので、返信が来た時だけpublic URLとtechnical summaryを記録する。 | private本文を貼らずにSearch Review LogかProvider Feedback Captureへ記録できる。 |
