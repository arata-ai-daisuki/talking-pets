...
| --- | --- | --- | --- |
| Local TTS approval follow-up | 歌澄 音羽 / 白瀬 怜奈 | Active | 依存追加やdownloadなしで、次にMaster承認が必要なprovider境界を確認する。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| Multilingual evidence queue check | 言守 詞葉 / 白瀬 怜奈 | outreachやprovider返信が来た時にfallback-only/provider-specificを即分類できるようにする。 | README claimを変えず、証跡が来た時の記録先が明確になる。 |
