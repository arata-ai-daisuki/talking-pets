...
| --- | --- | --- | --- |
| MeloTTS external runtime helper design | 歌澄 音羽 | Active | installer download/installへ進まず、detect/connect-only helper設計を固める。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| MeloTTS detect/connect-only implementation card | 歌澄 音羽 | T176で外部runtime helper境界を固めた後、実装するなら最小の検出/接続だけに切れる。 | helper実装、installer導線、README wordingのどれを先にPR化するかが決まる。 |
