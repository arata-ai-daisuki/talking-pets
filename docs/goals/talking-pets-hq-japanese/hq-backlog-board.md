...
| --- | --- | --- | --- |
| Scorecard evidence follow-up | 歌澄 音羽 / 白瀬 怜奈 | Active | Provider scorecardのgapが明確になったので、必要ならdesign note質問へ落とし込む。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| Outreach watch cadence | 星宮 未来 | 送信後は返信待ちで止まらず、手動ログと2週間ルールだけを保つ。 | sent/watch/reply laterの状態がSearch Review Logで追える。 |
| Multilingual intake follow-up | 言守 詞葉 | 外部証跡が届いたら、今回のhandling orderに沿ってverification-statusとprovider intakeを更新する。 | fallback-only/provider-specific/provider feedback intakeの混線がない。 |
