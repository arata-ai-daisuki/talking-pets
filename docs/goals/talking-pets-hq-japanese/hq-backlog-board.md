...
| --- | --- | --- | --- |
| MeloTTS next integration choice | 相庭 愛 | Active | health-only helper後、monitor health integration / installer detect-only / provider feedbackの次PR候補を選ぶ。 |

## Next

| Card | Owner | Why next | Done when |
| --- | --- | --- | --- |
| MeloTTS monitor health integration | 歌澄 音羽 | T179推奨。helperはあるので、monitor側にhealth-only導線だけつなげる。 | MeloTTS absentでもclean failureし、speech routing/default routingは変わらない。 |
